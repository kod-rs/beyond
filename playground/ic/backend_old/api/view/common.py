from playground.ic.backend_old.api.config.constants import INTERNAL_SERVER_ERROR_MESSAGE
...
def get_auth_err_response_template(request):
    response = {"auth": {"status": False}}
    return response


def check_request_contains(request, attribute,
                           raise_exception=True, default_attribute=None):
